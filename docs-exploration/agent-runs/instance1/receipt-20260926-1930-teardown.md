...
- **Action:** Verified deletion via `gcloud container clusters list --project="barni-cnrm-20260529"`
- **Result:**
  ```text
...
- **Changes:** Updated the `## Teardown` section to explicitly include `kubectl -n openrl-system delete workloads,resourceclaims,pvc --all --ignore-not-found` prior to deleting the cluster.
- **Reason:** Deleting the PVC while the cluster is still active triggers the Filestore CSI driver to cleanly remove the underlying Google Cloud Filestore instance, avoiding orphaned Filestore storage.
